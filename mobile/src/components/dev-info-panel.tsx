...
export interface DevInfoPanelProps {
  role: 'parent' | 'child';
}

// Inner panel implementation. All hooks live here so the `__DEV__`
// gate in the outer wrapper does not introduce a conditional
// hooks violation.
function DevInfoPanelImpl({ role }: DevInfoPanelProps) {
  ...
  const [notifProbe, setNotifProbe] = useState<string | null>(null);
  ...
  return (
    <View style={styles.card}>
      ...
      <Text style={styles.sectionSub}>Hidden. Only for the developer.</Text>
      ...
      <View style={styles.row}>
        ...
        <TouchableOpacity onPress={load} disabled={isRefreshing}>
          {isRefreshing ? (
...
export function DevInfoPanel({ role }: DevInfoPanelProps) {
  if (!__DEV__) return null;
  return <DevInfoPanelImpl role={role} />;
}
...
const styles = StyleSheet.create({
  ...
  divider: {
    // AGENTS.md forbids 1px dividers. Use vertical spacing instead.
  },
  sectionGap: {
    height: 12,
  },
