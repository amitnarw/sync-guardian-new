-- ============================================================
-- Safer advisory lock key in claim_pairing_token
--
-- The original implementation called:
--   pg_advisory_xact_lock(hashtextextended(...))
--
-- hashtextextended returns bigint and is not contractually bounded
-- to the signed 64-bit range on every input. On adversarial or
-- pathological inputs, the result can land outside the documented
-- range, potentially colliding with another parent's lock key.
--
-- Switch to the two-argument form pg_advisory_xact_lock(int4, int4),
-- where both arguments are hashtext() (int4) on disjoint inputs.
-- hashtext() is contractually a signed 32-bit integer, so the
-- combined 64-bit key space is always well-bounded.
-- ============================================================
...
BEGIN
  ...
  -- Two-argument form (int4, int4) gives us a 64-bit effective key
  -- space without overflow risk on adversarial inputs.
  PERFORM pg_advisory_xact_lock(
    hashtext('claim_limit:' || p_parent_user_id::text),
    hashtext('claim_limit_v2:' || p_parent_user_id::text)
  );
  ...
  -- Tier resolution (matches product rules and the shared entitlement
  -- module):
